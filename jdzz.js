...
!(async () => {
  $.tuanList = []
  await requireConfig();
  if (helpAuthor) await getAuthorShareCode('https://gitee.com/shy1ocks/updateTeam/raw/main/jd_zz.json');
  if (helpAuthor) await getAuthorShareCode('https://gitee.com/lxk030l/updateTeam/raw/master/shareCodes/jd_zz.json');
  if (!cookiesArr[0]) {
...
function readShareCode() {
  console.log(`开始`)
  return new Promise(async resolve => {
    $.get({url: `https://code.chlang.fun/api/vl/jd/jdzz/read/${randomCount}/`, 'timeout': 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)
          console.log(`${$.name} API请求失败，请检查网路重试`)
